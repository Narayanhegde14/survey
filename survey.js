...
function saveState(){ localStorage.setItem('ev_baas_survey_state', JSON.stringify(state)); }
function val(id){ return state[id]; }
function setVal(id, v){ state[id] = v; saveState(); }
...
function computeUsage(ans = state){
  ...
  return { monthlyKm, kmPerSwap, swapsNeededExact, swapsNeededCeil, kWhPerMonth, whPerKm, days };
}
...
function calcLiteEconomics(ans = state){
  ...
  const includedKWh = ECON.LITE_KWH_CAP ?? (ECON.LITE_INCLUDED_SWAPS * ECON.KWH_PER_SWAP);
  ...
  return { ...u, includedKWh, virtualIncludedSwaps, feeIncl, rateIncl, extraKWh, monthlyCost, subPerKm, sub3y };
}

// Ownership battery cost across the horizon
function calcOwnershipEconomics(ans = state){
  const u = computeUsage(ans);
  const ratePerKWh = (ECON.OWN_PACK_PRICE) / (ECON.OWN_PACK_KWH * ECON.OWN_CYCLES); // ₹/kWh (battery amortization only)
  const ownMonthly = u.kWhPerMonth * ratePerKWh;
  const ownPerKm = u.monthlyKm ? (ownMonthly / u.monthlyKm) : 0;
  return { monthlyKm: u.monthlyKm, kWhPerMonth: u.kWhPerMonth, ratePerKWh, ownMonthly, ownPerKm };
}
...
function toast(msg){
  ...
  t.textContent = msg;
  t.style.cssText = 'position:fixed; bottom:16px; left:50%; transform:translateX(-50%); background:#111936; border:1px solid rgba(255,255,255,.15); padding:10px 14px; border-radius:12px; color:#e5e7eb; z-index:9999; box-shadow:0 10px 30px rgba(0,0,0,.35)';
...
const DEFAULT_MEDIA_IMG = "https://lh3.googleusercontent.com/p/AF1QipODon_IrWq4LT3ycNfmLNLTA05I3ZYE_MjEDyXd=s1360-w1360-h1020";
...
function drawBarChart(canvas, labels, values){
  ...
  ctx.fillStyle = '#0b122a'; ctx.fillRect(0,0,W,H);
  ctx.strokeStyle='rgba(255,255,255,.08)'; ctx.strokeRect(0.5,0.5,W-1,H-1);

  const padL = 48, padR = 20, padT = 28, padB = 56;
  ...
  ctx.moveTo(padL, padT + chartH); ctx.lineTo(padL + chartW, padT + chartH); ctx.stroke();

  for(let i=0;i<n;i++){
    const v = Number(values[i])||0;
    const h = (v / max) * chartH;
    const x = x0 + i*slot, y = padT + chartH - h;

    const g = ctx.createLinearGradient(0, y, 0, y+h);
    g.addColorStop(0, '#6ee7ff'); g.addColorStop(1, '#a78bfa');
    ...
    ctx.fillStyle = 'rgba(255,255,255,.85)'; ctx.font = '12px system-ui,Segoe UI';
    const baseY = padT + chartH + 16;
    lines.forEach((ln, j)=>{
      ctx.fillText(ln, x + (barW - ctx.measureText(ln).width)/2, baseY + j*14);
    });
  }
  ...
  function wrapText(text, ctx, maxWidth, font, maxLines){
    ...
    for(const w of words){
      ...
      else { lines.push(cur || w); cur = (ctx.measureText(w).width > maxWidth) ? truncate(w) : w; }
      if(lines.length === maxLines) break;
    }
    if(cur && lines.length < maxLines) lines.push(cur);
    return lines;
    function truncate(str){
      let s = str; while(ctx.measureText(s + '…').width > maxWidth && s.length>1) s = s.slice(0,-1);
      return s + '…';
    }
  }
}
...
const components = {
  ...
  introContact: (s) => {
    ...
    div.innerHTML = `
      <div class="vstack">
        ...
        <div class="vstack">
          <label>Name <input type="text" id="${s.id}_name" value="${prev.name||''}" placeholder="Your name"/></label>
          <label>Phone <input type="tel" id="${s.id}_phone" value="${prev.phone||''}" placeholder="10-digit mobile"/></label>
          <label>Email <input type="email" id="${s.id}_email" value="${prev.email||''}" placeholder="you@example.com"/></label>
          <label>Age <input type="number" min="12" max="100" id="${s.id}_age" value="${prev.age||''}"/></label>
          <label>City / Area <input type="text" id="${s.id}_place" value="${prev.place||''}" placeholder="e.g., Indiranagar, Bengaluru"/></label>
          <label><input type="checkbox" id="${s.id}_consent" ${prev.consent? 'checked':''}/> I agree to be contacted only to share survey goodies and relevant EV/BaaS updates. No spam.</label>
        </div>
      </div>
      ${placeholderMedia()}
    `;
  ...
  radioGrid: (s) => {
    ...
    div.addEventListener('change', (e)=>{
      if(e.target.name===s.id){ setVal(s.id, e.target.value); validateAndToggle(s); }
    });
    return div;
  },
  ...
  checkboxGrid: (s) => {
    ...
    div.innerHTML = `
      <div class="vstack">
        ...
        ${s.allowOther? `<input type="text" id="${s.id}_other" placeholder="${s.otherPlaceholder||'Other (optional)'}" />`:''}
    ...
    function collect(e){
      const chosen = $$('input[type=checkbox]', div).filter(x=>x.checked).map(x=>x.value);
      if (s.max && chosen.length > s.max){
        if(e && e.target && e.target.type==='checkbox'){ e.target.checked = false; return; }
      }
      if(s.allowOther){
  ...
  fixedSumSliders: (s) => {
    ...
    if (Object.keys(prev).length === 0) {
      const n = keys.length;
      let base = Math.floor(TOTAL / n), rem = TOTAL - base*n;
      keys.forEach((k,i)=> data[k] = Math.min(MAX, base + (i<rem?1:0)));
      setVal(s.id, data);
    }
    ...
    div.innerHTML = `
      ...
      <div class="media"><canvas id="${s.id}_chart" width="560" height="320"></canvas></div>
    `;

    const canvas = $(`#${s.id}_chart`, div);

    function total(){ return keys.reduce((a,k)=> a + (Number(data[k]||0)), 0); }
    function updateUI(){
      drawBarChart(canvas, keys.map(k=>labels[k]), keys.map(k=>Number(data[k]||0)));
      keys.forEach(k=>{
    ...
    div.addEventListener('input', (e)=>{
      if(e.target.matches('input[type=range][data-key]')) setValue(e.target.dataset.key, e.target.value);
    });
    div.addEventListener('click', (e)=>{
      if(e.target.id === `${s.id}_even`){
        const n = keys.length; let base = Math.floor(TOTAL / n), rem = TOTAL - base*n;
        keys.forEach((k,i)=> data[k] = Math.min(MAX, base + (i<rem?1:0)));
        updateUI();
      }
    });
  ...
  ranges: (s) => {
          ...
          ${s.items.map(m=>{
            ...
            return `<div class="range">
              <label class="${m.required? 'req':''}">
                <span>${m.label}${m.key==='wh_per_km'? ' <span class="modechip" id="'+s.id+'_mode">City</span>':''}</span>
                <span><b id="${s.id}_${m.key}_val">${fmt(m, v)}</b></span>
              </label>
              <input aria-label="${m.label}" type="range" min="${m.min}" max="${m.max}" step="${m.step||1}" value="${v}" data-key="${m.key}"/>
              ${m.key==='wh_per_km'? '<div class="rangelabels"><span>Eco</span><span>City</span><span>Sports</span><span>Blaze</span></div>':''}
            </div>`;
          }).join('')}
        </div>
      </div>
    ...
    function fmt(m, v){ return m.currency ? rupee.format(v) : (m.suffix ? `${v}${m.suffix}` : `${v}`); }
    ...
    function renderSummary(){
      ...
      if(mc){ mc.textContent = label; mc.className = `modechip ${cls}`; }
      const htmlHelp = (s.help && s.help.__current) ? s.help.__current : (s.infoDefaultHTML || defaultHelpBox());
      box.innerHTML = `
        <div class="helpbox">
          ...
          <div class="statgrid">
            ...
            <div class="stat"><div class="kv"><span class="k">Energy use</span><span class="v">${u.kWhPerMonth.toFixed(1)} kWh</span></div></div>
            <div class="stat"><div class="kv"><span class="k">Swaps needed</span><span class="v">${Math.ceil(u.swapsNeededExact)}/mo</span></div></div>
            <div class="stat"><div class="kv"><span class="k">~ km per swap</span><span class="v">${u.kmPerSwap.toFixed(1)}</span></div></div>
          </div>
          <div class="help-card">
            <div class="hc-title">Mode</div>
            <div class="kv"><span class="v"><span class="modechip ${cls}">${label}</span> · ${u.whPerKm.toFixed(1)} Wh/km</span></div>
    ...
    function setHelpWithSummary(key){
      if(!s.help){ renderSummary(); return; }
  ...
  subscriptionAppeal: (s) => {
    ...
    div.innerHTML = `
      ...
      <div class="subgrid twoCols">
        ...
        <div class="media"><canvas id="${s.id}_chart" width="700" height="360"></canvas></div>
      </div>

      <!-- Second row: monthly usage under options (left) and Home charging full-width below -->
      <div class="subgrid twoCols">
        <div class="infocard" id="${s.id}_assump" style="grid-column:1/2;"></div>
      ...
      <div class="smallnote fullwidth" id="${s.id}_note" style="margin-top:8px;"></div>
    `;

    const fmtR = (n)=> new Intl.NumberFormat('en-IN',{style:'currency',currency:'INR',maximumFractionDigits:0}).format(Math.round(Number(n)||0));
    ...
    function onInputChange(e){
      ...
      if(t.id === `${s.id}_tariff` || t.id === `${s.id}_loss`){
        const tval = Number(document.getElementById(`${s.id}_tariff`)?.value||8);
        const lval = Number(document.getElementById(`${s.id}_loss`)?.value||12);
    ...
    div.addEventListener('change', onInputChange);
      ...
      const includedKm = ECON.LITE_KWH_CAP * 1000 / (u.whPerKm || ECON.WH_PER_KM);
      $(`#${s.id}_assump`, div).innerHTML = `
        <div class="title-row">Your monthly usage</div>
        <div class="kv"><span class="k">Monthly km</span><span class="v">${fmtInt(u.monthlyKm)}</span></div>
        <div class="kv"><span class="k">~ km per swap <span class="tip" tabindex="0" data-tip="Battery has ~3.0 kWh installed but you are billed for 2.5 kWh usable per swap. All calculations use 2.5 kWh/swap.">ℹ︎</span></span><span class="v">${fmt1(u.kmPerSwap)}</span></div>
        <div class="kv"><span class="k">Swaps needed</span><span class="v">${u.swapsNeededCeil}/mo</span></div>
        <div class="kv"><span class="k">Energy use</span><span class="v">${fmt1(u.kWhPerMonth)} kWh/mo</span></div>
        <div class="kv"><span class="k">Horizon</span>
          <span class="v">
            <input type="range" id="${s.id}_yrs" min="1" max="10" step="1" value="${Number(val('horizon_years') || ECON.HORIZON_YEARS || 3)}" style="width:160px; vertical-align:middle"/>
            <b id="${s.id}_yrs_lbl">${Number(val('horizon_years') || ECON.HORIZON_YEARS || 3)}</b> yrs
          </span>
        </div>
      `;

      // --- Lite block (as before) ---
      const liteStatus = lite.extraKWh <= 0
        ? '<span class="status ok">Within cap</span>'
        : `<span class="status warn">Exceeds by ${fmt1(lite.extraKWh)} kWh</span>`;

      const rateFixedIncl = ECON.EXTRA_RATE_FIXED_PER_KWH_EXGST * (1 + ECON.GST_RATE);
      ...
      let liteRec = '';
      const minMonthly = Math.min(lite.monthlyCost, basicMonthlyTotal, advMonthlyTotal);
      if (minMonthly === lite.monthlyCost) liteRec = '<span class="status ok">Best for current usage</span>';
      else if (minMonthly === basicMonthlyTotal) liteRec = '<span class="status info">Basic likely cheaper</span>';
      ...
      $(`#${s.id}_lite`, div).innerHTML = `
        ...
        <div class="sub">Cap: ${ECON.LITE_KWH_CAP} kWh · Fee: ${fmtR(lite.feeIncl)}/mo</div>
        <div class="kv"><span class="k">Status</span><span class="v">${liteStatus}</span></div>
        <div class="kv"><span class="k">Over-cap rate</span><span class="v">${fmtR(lite.rateIncl)} /kWh</span></div>
        <div class="kv"><span class="k">Est. extra this month</span><span class="v">${lite.extraKWh>0? fmtR(lite.extraKWh * lite.rateIncl) : '—'}</span></div>
        <div class="kv"><span class="k">Total per month</span><span class="v">${fmtR(lite.monthlyCost)}</span></div>
        <div class="kv"><span class="k">Est. ₹/km</span><span class="v">${fmt2(lite.subPerKm)} ₹/km</span></div>
      `;
      ...
      function perKm(total){ return u.monthlyKm ? (total / u.monthlyKm) : 0; }

      $(`#${s.id}_basic`, div).innerHTML = `
        <div class="title-row">Honda <span class="tag">Basic</span></div>
        <div class="sub">Cap: ${ECON.BASIC_KWH_CAP} kWh · Fee: ${fmtR(ECON.BASIC_FEE)}/mo</div>
        <div class="kv"><span class="k">Status</span><span class="v"><span class="status ${basic.within?'ok':'warn'}">${basic.within? 'Within cap' : 'Exceeds by ' + fmt1(basic.over) + ' kWh'}</span></span></div>
        <div class="kv"><span class="k">Over-cap rate</span><span class="v">${fmtR(rateFixedIncl)} /kWh</span></div>
        <div class="kv"><span class="k">Est. extra this month</span><span class="v">${basicOver>0? fmtR(basicOver * rateFixedIncl) : '—'}</span></div>
        <div class="kv"><span class="k">Total per month</span><span class="v">${fmtR(basicMonthlyTotal)}</span></div>
        <div class="kv"><span class="k">Est. ₹/km</span><span class="v">${fmt2(perKm(basicMonthlyTotal))} ₹/km</span></div>
      `;

      $(`#${s.id}_adv`, div).innerHTML = `
        <div class="title-row">Honda <span class="tag">Advanced</span></div>
        <div class="sub">Cap: ${ECON.ADV_KWH_CAP} kWh · Fee: ${fmtR(ECON.ADV_FEE)}/mo</div>
        <div class="kv"><span class="k">Status</span><span class="v"><span class="status ${adv.within?'ok':'warn'}">${adv.within? 'Within cap' : 'Exceeds by ' + fmt1(adv.over) + ' kWh'}</span></span></div>
        <div class="kv"><span class="k">Over-cap rate</span><span class="v">${fmtR(rateFixedIncl)} /kWh</span></div>
        <div class="kv"><span class="k">Est. extra this month</span><span class="v">${advOver>0? fmtR(advOver * rateFixedIncl) : '—'}</span></div>
        <div class="kv"><span class="k">Total per month</span><span class="v">${fmtR(advMonthlyTotal)}</span></div>
        <div class="kv"><span class="k">Est. ₹/km</span><span class="v">${fmt2(perKm(advMonthlyTotal))} ₹/km</span></div>
      `;

      // --- Comparator and horizon logic (reused from previous version) ---
      const compareState = Object.assign({ home_tariff: 8, home_loss_pc: 12 }, (val('compare')||{}));
      ...
      const feeB = ECON.BASIC_FEE, capB = ECON.BASIC_KWH_CAP;
      const feeA = ECON.ADV_FEE,   capA = ECON.ADV_KWH_CAP;
      ...
      function beDetails(U){
        ...
        return `${fmt1(U)} kWh/mo (~${fmtInt(Math.round(km))} km, ${fmt1(swaps)} swaps) · ${delta}`;
      }
...
//       `;
      ...
      ['_tariff','_loss'].forEach(suf=>{
        ...
        if(el){
          el.addEventListener('input', ()=>{
            const t = Number(document.getElementById(`${s.id}_tariff`).value||8);
            const l = Number(document.getElementById(`${s.id}_loss`).value||12);
      ...
      const packsNeededStep = lifetimeKWhPerPack > 0 ? Math.ceil(totalKWhHorizon / lifetimeKWhPerPack) : 0;
      const investedStep = packsNeededStep * ECON.OWN_PACK_PRICE;
      const lastPackUsedKWh = Math.max(0, totalKWhHorizon - Math.max(0, packsNeededStep-1) * lifetimeKWhPerPack);
      const lastPackFracUsed = packsNeededStep ? Math.min(1, lastPackUsedKWh / lifetimeKWhPerPack) : 0;
      const usedCostStep = (Math.max(0, packsNeededStep-1) + lastPackFracUsed) * ECON.OWN_PACK_PRICE;
      const wasteCostStep = Math.max(0, investedStep - usedCostStep);

      const bars = [
        { label: `Lite (${years}y)`, value: Math.round(lite.monthlyCost * 12 * years) },
        { label: `Basic (${years}y)`, value: Math.round(basicMonthlyTotal * 12 * years) },
        { label: `Advanced (${years}y)`, value: Math.round(advMonthlyTotal * 12 * years) },
        { label: `Ownership — used (${years}y)`, value: Math.round(usedCostStep) },
        { label: `Ownership — leftover (${years}y)`, value: Math.round(wasteCostStep) }
      ];
      const canvas = $(`#${s.id}_chart`, div);
      drawBarChart(canvas, bars.map(b=>b.label), bars.map(b=>b.value));

      // Note
      $(`#${s.id}_note`, div).innerHTML =
        `Ownership amortization uses ₹${(ECON.OWN_PACK_PRICE).toLocaleString()} per ${ECON.OWN_PACK_KWH} kWh pack over ${ECON.OWN_CYCLES} cycles (~₹${Math.round(ECON.OWN_PACK_PRICE/(ECON.OWN_PACK_KWH*ECON.OWN_CYCLES))}/kWh). Bars reflect monthly totals × ${Number(val('horizon_years') || ECON.HORIZON_YEARS || 3)} years at your current usage.`;
    }
      ...
      <div class="vstack">
        ...
        <div class="vstack">
          <label class="${s.required?.includes('name')? 'req':''}">Name
            <input type="text" id="${s.id}_name" value="${prev.name||''}" placeholder="Optional"/>
          </label>
          <label class="${s.required?.includes('email')? 'req':''}">Email
            <input type="email" id="${s.id}_email" value="${prev.email||''}" placeholder="Optional"/>
          </label>
          <label>Phone
            <input type="tel" id="${s.id}_phone" value="${prev.phone||''}" placeholder="Optional"/>
          </label>
          <label><input type="checkbox" id="${s.id}_consent" ${prev.consent? 'checked':''}/> I consent to be contacted about EV/BaaS updates.</label>
        </div>
      </div>
      ${placeholderMedia()}
    `;
  ...
  review: (s) => {
    ...
    div.innerHTML = `
      <div class="vstack">
        ...
        <p class="subtitle">${s.subtitle||''}</p>
        <div class="summary">
          <div><span class="tag">Vehicle</span> <b>${payload.current_vehicle||'—'}</b></div>
          <div><span class="tag">Daily Commute</span> ${payload.commute?.daily_km ?? '—'} km • Longest ${payload.commute?.longest_km ?? '—'} km</div>
          <div><span class="tag">Subscription Appeal</span> ${payload.subscription_appeal || '—'}</div>
    ...
    setTimeout(()=>{
      ...
      const labels = Object.keys(pr), vals = Object.values(pr).map(Number);
      if(labels.length) drawBarChart($('#reviewChart', div), labels, vals);
      $('#downloadBtn', div).addEventListener('click', ()=> download('ev-baas-survey.json', JSON.stringify(payload, null, 2)));
...
const slides = [
  {
    id: 'intro_contact', required: false, component: 'introContact',
    title: 'Let’s get you your goodies 🎁',
    subtitle: 'We will not spam. We need these details only to share digital goodies and key takeaways at the end.'
  },
  {
    id: 'current_vehicle', required: true, component: 'radioGrid',
    title: 'What best describes your current vehicle situation?',
    options: ['I own an EV', 'I own a petrol/diesel vehicle', 'I own both', "I don't own a vehicle"],
  },
  {
    id: 'ev_experience', required: false, component: 'checkboxGrid',
    title: 'If you use an EV, what are your biggest pain points?',
    subtitle: '(Choose all that apply)',
    options: ['Limited range', 'Charging downtime', 'Charger reliability', 'Battery degradation', 'Service/repairs', 'Cost of replacement battery'],
    allowOther: true,
    otherPlaceholder: 'Other (optional)',
    condition: (ans)=> ['I own an EV','I own both'].includes(ans.current_vehicle)
  },
  {
    id: 'why_not_ev', required: false, component: 'radioGrid',
    title: 'Main reason you haven’t chosen an EV yet?',
    options: ['Upfront cost is high', 'Range anxiety', 'Charging access', 'Waiting for better models', 'Battery life concerns', 'Other'],
    condition: (ans)=> ['I own a petrol/diesel vehicle', "I don't own a vehicle"].includes(ans.current_vehicle)
  },
  {
    id: 'commute', required: true, component: 'ranges',
    title: 'Tell us about your commute',
    items: [
      { key:'daily_km', label:'Average daily distance', min:0, max:150, step:1, suffix:' km', default: 15, required:true },
      { key:'days_used', label:'Days used per month', min:0, max:31, step:1, suffix:' days', default: 26, required:true },
      { key:'longest_km', label:'Longest typical one-way trip (per month)', min:0, max:300, step:5, suffix:' km', default: 60 },
      { key:'wh_per_km', label:'Typical energy use (consumption)', min:21.1, max:51.4, step:0.1, suffix:' Wh/km', default: 32, required:true }
    ],
    help: {
      wh_per_km: `
        <div class="vstack">
  ...
  {
    id: 'priorities', required: true, component: 'fixedSumSliders',
    title: 'What matters most to you?',
    subtitle: 'Distribute exactly 20 points (max 5 per slider). The chart updates live.',
    total: 20, maxPer: 5,
    metrics: [
  ...
  {
    id: 'performance', required: true, component: 'ranges',
    title: 'Performance expectations for your ideal EV',
    items: [
      { key:'accel_0_40', label:'0–40 km/h time', min:2, max:12, step:0.5, suffix:' s', default: 5, required:true },
      { key:'top_speed', label:'Top speed', min:45, max:120, step:5, suffix:' km/h', default: 80 },
      { key:'hill', label:'Hill-climb priority', min:0, max:10, step:1, suffix:' /10', default: 5 },
    ],
    help: {
      accel_0_40: `
        <div class="vstack">
  ...
  {
    id: 'cost', required: true, component: 'ranges',
    title: 'Cost expectations',
    items: [
      { key:'upfront', label:'Budget for vehicle (on-road)', min:30000, max:300000, step:5000, currency: true, default: 120000, required:true },
      { key:'monthly', label:'Expected monthly operating cost (fuel/energy + service)', min:200, max:6000, step:100, currency: true, default: 1200 },
    ],
    help: {
      upfront: `
        <div class="vstack">
  ...
  {
    id: 'conversion_concerns', required: false, component: 'checkboxGrid',
    title: 'What are your top concerns about converting an ICE vehicle to electric?',
    subtitle: 'Select up to two.',
    options: ['Conversion cost', 'Reliability after conversion', 'Performance compared to petrol', 'Availability of service/support'],
    allowOther: true, otherPlaceholder: 'Other (e.g., Range)',
    max: 2
  },
  {
    id: 'subscription_appeal', required: true, component: 'subscriptionAppeal',
    title: 'How appealing is a monthly battery subscription?',
    subtitle: 'We compare Honda’s Lite plan (with extra swaps if you exceed 12/mo) vs owning the battery for 3 years.',
    options: ['Very appealing', 'Somewhat appealing', 'Neutral', 'Not appealing']
  },
  {
    id: 'review', required: false, component: 'review',
    title: 'Review & Submit',
...
function buildPayload(){
  return {
    ...
    meta: { userAgent: navigator.userAgent, ts: new Date().toISOString(), version: 'split-0.4-honda' }
  };
}
...
let idx = Math.min(Number(localStorage.getItem('ev_baas_slide_idx')||0), flow.length-1);

function go(i){ idx = Math.max(0, Math.min(i, flow.length-1)); localStorage.setItem('ev_baas_slide_idx', idx); render(); }
...
function validateAndToggle(s){
  ...
  if (s.component === 'fixedSumSliders'){
    ...
    validationMsg.textContent = isOk ? '' : `Please allocate exactly ${requiredTotal} points (current total ${total}).`;
    return;
  }

  if(s.required){
    if(v === undefined || v === null || v === '' || (Array.isArray(v) && v.length===0)) ok = false;
    if(typeof v === 'object' && !Array.isArray(v)){
      if(s.component==='ranges' && s.items){
        ok = s.items.every(item => !item.required || (v[item.key]!==undefined && v[item.key]!==null));
      }
    }
  }
  nextBtn.disabled = !ok;
...
nextBtn.addEventListener('click', ()=>{
  if(idx === flow.length-1){ toast('Thanks! Your responses are ready.'); return; }
  go(idx+1);
});
...
document.addEventListener('keydown', (e)=>{
  ...
  if(e.key==='ArrowRight' || e.key===' ' || e.key==='Enter'){ if(!nextBtn.disabled) nextBtn.click(); }
...
if (typeof components === 'object') {
  components.brandStars = (s) => {
    const brands = s.brands || ["Ather","Ola","TVS","Bajaj","Hero","Honda"];
    const prev = val(s.id) || {};
    const div = document.createElement('div');
    div.className = 'content';
    ...
    function paint(){
      $$('.star', div).forEach(btn=>{
        const b = btn.dataset.brand, n = Number(btn.dataset.val);
        const sel = Number((prev||{})[b] || 0);
        btn.style.opacity = n <= sel ? 1 : 0.35;
        btn.style.filter = n <= sel ? 'drop-shadow(0 2px 6px rgba(0,0,0,.35))' : 'grayscale(0.4)';
      });
    }
    div.addEventListener('click', e=>{
...
try {
  const idxReview = (typeof slides !== 'undefined')
    ? Math.max(0, slides.findIndex(s => s.id === 'review'))
    : -1;
  const insertAt = (idxReview >= 0 ? idxReview : Math.max(0, (slides?.length||1) - 1));
...
function fixSlide9Layout() {
  ...
  const usage = Array.from(s9.querySelectorAll('.card,.panel,.box,.section,.container'))
    .find(el => /Your monthly usage/i.test(el.textContent || ''));
  if (usage) usage.classList.add('usage-card');

  // Tag: Home charging vs plans
  const home = Array.from(s9.querySelectorAll('.card,.panel,.box,.section,.container'))
  ...
  const optionsContainer =
    ...
    Array.from(s9.children).find(el => el.querySelector('input[type="radio"]'));
  ...
  const chartCard =
    ...
    Array.from(s9.querySelectorAll('.card,.panel,.box,.section,.container'))
